...
**The Ranking Formula:**
`Total Score = Lexical Match Score + Popularity Bonus`

1. **Lexical Match (`ts_rank_cd`):** Standard density ranking based on A/B keyword matches.
2. **Popularity Bonus:** Adds `(hot_score * 0.1)` to the total score so highly upvoted questions rise to the top.
